...
movCat:AddItem("Driller", "Moves on the grid but drills through its obstacles instead of pushing them. Cannot drill through certain blocks", 57)
...
weightCat:AddItem("Weight", "Stops 1 mover", 21)

local destroyCat = Toolbar:AddCategory("Destroyers", "Cells that destroy other cells", "textures/enemy.png")
destroyCat:AddItem("Enemy", "Destroys one cell that moves into it then destroys itself", 12)
destroyCat:AddItem("Strong Enemy", "Just like an enemy, but instead of destroying self, it turns itself into a normal enemy. Basically an enemy that can destroy two cells", 23)
destroyCat:AddItem("Trash", "Destroys all cells that move into it", 11)
destroyCat:AddItem("Demolisher", "Trash cell, except in addition to destroying what fell into it, it destroys cells adjacent to it as well", 50)
...
procCat:AddItem("OR gate", "Performs OR operation on its data. True if at least one input is detected", 31)
procCat:AddItem("AND gate", "Performs AND operation on its data. True if both inputs are detected", 32)
procCat:AddItem("XOR gate", "Performs XOR operation on its data. True if exactly one input is detected", 33)
procCat:AddItem("NOR gate", "Performs NOR operation on its data. True if no input is detected", 34)
procCat:AddItem("NAND gate", "Performs NAND operation on its data. True if up to one input is detected", 35)
procCat:AddItem("XNOR gate", "Performs XNOR operation on its data. True if either no input or both inputs are detected", 36)
...
uniqueCat:AddItem("Bias", "Messes with the movers biases. Basically acts as a Mover if pushed from behind", 41)
